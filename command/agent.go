...
import (
	...
	"github.com/jiangjiali/vault/command/agent/auth"
	"github.com/jiangjiali/vault/command/agent/auth/jwt"
	"github.com/jiangjiali/vault/command/agent/cache"
	"github.com/jiangjiali/vault/command/agent/config"
	"github.com/jiangjiali/vault/command/agent/sink"
	"github.com/jiangjiali/vault/command/agent/sink/file"
	"github.com/jiangjiali/vault/command/agent/sink/inmem"
	...
	"github.com/jiangjiali/vault/sdk/helper/kr/pretty"
...
func (c *AgentCommand) Run(args []string) int {
	...
	if xxconfig.AutoAuth != nil {
		...
		authConfig := &auth.AConfig{
			Logger:    c.logger.Named(fmt.Sprintf("auth.%s", xxconfig.AutoAuth.Method.Type)),
			MountPath: xxconfig.AutoAuth.Method.MountPath,
			Config:    xxconfig.AutoAuth.Method.Config,
		}
		...
		if err != nil {
			c.UI.Error(errwrap.Wrapf(fmt.Sprintf("Error creating %s auth method: {{err}}", xxconfig.AutoAuth.Method.Type), err).Error())
			return 1
		}
	}
