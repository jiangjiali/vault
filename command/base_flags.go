...
func (i *uintValue) Get() interface{} { return *i.target }
...
func (f *FlagSet) VarFlag(i *VarFlag) {
	...
	if len(i.Aliases) > 0 {
		...
		case 1:
			aliases = sentence[0]
		case 2:
			aliases = sentence[0] + " 和 " + sentence[1]
		default:
			sentence[len(sentence)-1] = "和 " + sentence[len(sentence)-1]
			aliases = strings.Join(sentence, "，")
		}

		usage += fmt.Sprintf(" 别名为%s。", aliases)
	}

	if i.Default != "" {
		usage += fmt.Sprintf(" 默认值是%s。", i.Default)
	}

	if i.EnvVar != "" {
		usage += fmt.Sprintf(" 可以通过%s环境变量指定。", i.EnvVar)
	}
