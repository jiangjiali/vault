...
func Handler(ctx context.Context, logger hclog.Logger, proxier Proxier, inmemSink sink.Sink) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if token == "" && inmemSink != nil {
			...
			token = inmemSink.(sink.SReader).Token()
		}
		...
		err = processTokenLookupResponse(logger, inmemSink, req, resp)
		if err != nil {
...
// response.
func processTokenLookupResponse(logger hclog.Logger, inmemSink sink.Sink, req *SendRequest, resp *SendResponse) error {
	// If auto-auth token is not being used, there is nothing to do.
	if inmemSink == nil {
		return nil
	}
	autoAuthToken := inmemSink.(sink.SReader).Token()
