...
import (
	...
	"github.com/jiangjiali/vault/command/agent/cache/cachememdb"
	"github.com/jiangjiali/vault/sdk/helper/base62"
	"github.com/jiangjiali/vault/sdk/helper/consts"
	"github.com/jiangjiali/vault/sdk/helper/cryptoutil"
	...
	nshelper "github.com/jiangjiali/vault/sdk/helper/namespace"
...
func (c *LeaseCache) Send(ctx context.Context, req *SendRequest) (*SendResponse, error) {
	...
	xxnamespace := req.Request.Header.Get(consts.NamespaceHeaderName)
...
func deriveNamespaceAndRevocationPath(req *SendRequest) (string, string) {
	...
	for _, pathToCheck := range revocationPaths {
		...
		if xxnamespace == "root/" {
			xxnamespace = namespaceInPath
		} else {
			xxnamespace = xxnamespace + namespaceInPath
		}

		return xxnamespace, fmt.Sprintf("/v1%s", nonVersionedPath[i:])
	}

	return xxnamespace, fmt.Sprintf("/v1%s", nonVersionedPath)
}
...
func (c *LeaseCache) RegisterAutoAuthToken(token string) error {
	...
	xxnamespace, err := base62.Random(5)
	if err != nil {
		return err
	}
