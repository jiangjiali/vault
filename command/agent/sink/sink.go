...
import (
	...
	"github.com/jiangjiali/vault/sdk/helper/dhutil"
...
func (ss *SServer) Run(ctx context.Context, incoming chan string, sinks []*SConfig) {
	...
	writeSink := func(currSink *SConfig, currToken string) error {
...
func (s *SConfig) encryptToken(token string) (string, error) {
	var aesKey []byte
	var err error
...
func (s *SConfig) wrapToken(client *api.Client, wrapTTL time.Duration, token string) (string, error) {
