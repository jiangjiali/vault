...
import (
	...
	"github.com/jiangjiali/vault/builtin/registry"
	"github.com/jiangjiali/vault/command/server"
	"github.com/jiangjiali/vault/sdk/helper/armon/metrics"
	"github.com/jiangjiali/vault/sdk/helper/armon/metrics/circonus"
	"github.com/jiangjiali/vault/sdk/helper/armon/metrics/datadog"
	"github.com/jiangjiali/vault/sdk/helper/armon/metrics/prometheus"
	"github.com/jiangjiali/vault/sdk/helper/complete"
	"github.com/jiangjiali/vault/sdk/helper/errwrap"
	"github.com/jiangjiali/vault/sdk/helper/jsonutil"
	"github.com/jiangjiali/vault/sdk/helper/logging"
	"github.com/jiangjiali/vault/sdk/helper/metricsutil"
	"github.com/jiangjiali/vault/sdk/helper/mitchellh/cli"
	"github.com/jiangjiali/vault/sdk/helper/mlock"
	"github.com/jiangjiali/vault/sdk/helper/multierror"
	"github.com/jiangjiali/vault/sdk/helper/namespace"
	"github.com/jiangjiali/vault/sdk/helper/parseutil"
	"github.com/jiangjiali/vault/sdk/helper/reload"
	...
	serverseal "github.com/jiangjiali/vault/command/server/seal"
	vaulthttp "github.com/jiangjiali/vault/http"
	gatedwriter "github.com/jiangjiali/vault/sdk/helper/gated-writer"
	log "github.com/jiangjiali/vault/sdk/helper/hclutil/hclog"
	vaultseal "github.com/jiangjiali/vault/vault/seal"
)
...
使用: vault server [选项]
  ...
  此命令启动响应API请求的安全库服务器。默认情况下，
  安全库将以“密封”状态启动。安全库群集必须在使用前初始化，
  通常通过“vault operator init”命令进行初始化。
  每个安全库服务器还必须使用“vault operator unseal”命令
  或API解封，然后服务器才能响应请求。
...
func (c *ServerCommand) Flags() *FlagSets {
	...
	f.StringSliceVar(&StringSliceVar{
		...
		Usage: "配置文件或配置文件目录的路径。可以多次指定此标志以加载多个配置。如果路径是一个目录，则加载以.hcl或.json结尾的所有文件。",
	})
	...
	f.StringVar(&StringVar{
		...
		Usage:      "日志详细级别。支持的值（按详细顺序）为“trace”、“debug”、“info”、“warn”和“err”。",
	})
	...
	f.BoolVar(&BoolVar{
		Name:   "dev",
		Target: &c.flagDev,
		Usage:  "启用开发模式。在此模式下，安全库在内存中运行并开始解封。顾名思义，不要在生产环境中运行“dev”模式。",
	})
...
func (c *ServerCommand) Run(args []string) int {
	...
	coreConfig := &vault.CoreConfig{
		...
		BuiltinRegistry:           registry.Registry,
...
ClusterSynthesisComplete:
	...
	for i, lnConfig := range config.Listeners {
		...
		var maxRequestDuration = vault.DefaultMaxRequestDuration
...
`)
	...
	if c.flagDev && !c.flagDevSkipInit {
		...
		c.UI.Warn(wrapAtLength(
			"警告！已启用开发模式！在此模式下，Vault完全在内存中运行，" +
				"并使用单个解封密钥开始解封。\n" +
				"根令牌已通过CLI验证，因此您可以立即开始使用。"))
		c.UI.Warn("")
		c.UI.Warn("您可能需要设置以下环境变量：")
		c.UI.Warn("")
		...
		if len(init.SecretShares) > 0 {
			c.UI.Warn("")
			c.UI.Warn(wrapAtLength(
				"如果您要密封/解封保险库或重新验证，则会在下面显示解封密钥和根令牌。"))
			c.UI.Warn("")
			c.UI.Warn(fmt.Sprintf("启封钥匙: %s", base64.StdEncoding.EncodeToString(init.SecretShares[0])))
		}

		if len(init.RecoveryShares) > 0 {
			c.UI.Warn("")
			c.UI.Warn(wrapAtLength(
				"恢复密钥和根令牌显示在下面，以防您要密封/解封保管库或重新验证。"))
			c.UI.Warn("")
			c.UI.Warn(fmt.Sprintf("恢复密钥: %s", base64.StdEncoding.EncodeToString(init.RecoveryShares[0])))
		}

		c.UI.Warn(fmt.Sprintf("根令牌: %s", init.RootToken))
	...
	for _, ln := range lns {
		...
		if readHeaderTimeoutInterface, ok := ln.config["http_read_header_timeout"]; ok {
			...
			xxserver.ReadHeaderTimeout = readHeaderTimeout
		}
		...
		if readTimeoutInterface, ok := ln.config["http_read_timeout"]; ok {
			...
			xxserver.ReadTimeout = readTimeout
		}
		...
		if writeTimeoutInterface, ok := ln.config["http_write_timeout"]; ok {
			...
			xxserver.WriteTimeout = writeTimeout
		}
		...
		if idleTimeoutInterface, ok := ln.config["http_idle_timeout"]; ok {
			...
			xxserver.IdleTimeout = idleTimeout
		}
	...
	if sealConfigError != nil {
		...
		if init {
			c.UI.Error("已初始化安全库，但无法加载任何密封密钥")
			return 1
		}
	}
	...
	if newCoreError != nil {
		c.UI.Warn(wrapAtLength(
			"警告！初始化期间发生非致命错误。请检查日志以了解更多信息。"))
		c.UI.Warn("")
	}

	// Output the header that the server has started
	if !c.flagCombineLogs {
		c.UI.Output("==> 服务器已启动！日志数据将在下面流式传输：\n")
	}
	...
	*/
	...
	if len(fanout) > 1 {
		...
		if !telConfig.DisableHostname {
			c.UI.Warn("telemetry.disable_hostname 已设置为false。对于普罗米修斯，建议的设置为true，以避免命名错误的度量。")
		}
	} else {
	...
	for {
		...
		if err == nil {
			if migrationStatus != nil {
				...
				c.UI.Error(wrapAtLength(fmt.Sprintf("错误！正在进行存储迁移（已启动：%s）。"+
					"在迁移完成之前，将阻止服务器启动。"+
					"使用“vault operator migrate-reset”强制清除移植锁定。", startTime)))
				return true
			}
			return false
		}
