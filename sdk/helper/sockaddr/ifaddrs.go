...
func IfAddrMath(operation, value string, inputIfAddr IfAddr) (IfAddr, error) {
	...
	case "network":
		...
		case TypeIPv4:
			...
			if i >= 0 {
				...
				wrappedMask = 1 + i + int64(^ipv4.Mask)
			}
	...
	case "mask":
		...
		case TypeIPv6:
			...
			return IfAddr{
				SockAddr: IPv6Addr{
					Address: maskedIpv6BigInt,
					Mask:    maskedIpv6MaskBigInt,
				},
				Interface: inputIfAddr.Interface,
			}, nil
		default:
...
// IPv6 connected host, submit an issue on github.com/jiangjiali/vault/sdk/sockaddr with
