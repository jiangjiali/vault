...
import (
	...
	"github.com/jiangjiali/vault/sdk/helper/compressutil"
	...
	"github.com/jiangjiali/vault/sdk/helper/memdb"
	...
	"github.com/jiangjiali/vault/sdk/helper/wrapping"
...
func (b *SystemBackend) handlePluginCatalogUpdate(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	...
	asha256 := d.Get("sha256").(string)
	if asha256 == "" {
		asha256 = d.Get("sha_256").(string)
		if asha256 == "" {
	...
	sha256Bytes, err := hex.DecodeString(asha256)
	if err != nil {
...
func (b *SystemBackend) handleAuditHash(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	...
	ahas, err := b.Core.auditBroker.GetHash(ctx, path, input)
	if err != nil {
...
// responseWrappingUnwrap will read the stored response in the public and
...
func (b *SystemBackend) responseWrappingUnwrap(ctx context.Context, te *logical.TokenEntry, thirdParty bool) (string, error) {
	...
	if responseRaw == nil {
		return "", fmt.Errorf("no response found inside the public")
	}
	response, ok := responseRaw.(string)
	if !ok {
		return "", fmt.Errorf("could not decode response inside the public")
	}
...
func (b *SystemBackend) handleWrappingRewrap(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	...
	if response == nil {
		return nil, fmt.Errorf("no response found inside the public")
	}
...
func (b *SystemBackend) pathRandomWrite(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	...
	randBytes, err := xxuuid.GenerateRandomBytes(bytes)
	if err != nil {
		return nil, err
	}
...
west coast.
	...
	"mount_path": {
		`The path to mount to. Example: "userpass/east"`,
		"",
	},
...
an example, "prod/userpass/" might be the AWS logical backend, and due to
a change in the "ops" policy, we may want to invalidate all the secrets
generated. We can do a revoke prefix at "prod/userpass/ops" to revoke all
...
all matching leases.
	...
	"revoke-prefix-path": {
		`The path to revoke keys under. Example: "prod/userpass/ops"`,
		"",
	},
...
it.`,
	...
	"unwrap": {
		...
		`Unwraps a response-wrapped token. Unlike simply reading from public/response,
...
This path responds to the following HTTP methods.
	...
	"leases-list-prefix": {
		`The path to list leases under. Example: "userpass/creds/deploy"`,
		"",
	},
	"plugin-reload": {
