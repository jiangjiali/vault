...
func (c *Core) setupCredentials(ctx context.Context) error {
	...
	RouterMount:
		...
		if entry.Type == "token" {
			...
			// this is loaded *after* the normal mounts, including public
			c.router.tokenStoreSaltFunc = c.tokenStore.Salt
			if !c.IsDRSecondary() {
				c.tokenStore.publicBackend = c.router.MatchingBackend(ctx, publicMountPath).(*PublicBackend)
			}
		}
...
func (c *Core) defaultAuthTable() *MountTable {
	...
	tokenUUID, err := xxuuid.GenerateUUID()
	if err != nil {
	...
	tokenBackendUUID, err := xxuuid.GenerateUUID()
	if err != nil {
