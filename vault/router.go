...
func (r *Router) routeCommon(ctx context.Context, req *logical.Request, existenceCheck bool) (*logical.Response, bool, bool, error) {
	...
	case strings.HasPrefix(originalPath, publicMountPath):
		...
		if te.Type != logical.TokenTypeService {
			return logical.ErrorResponse(`public operations are only supported by "service" type tokens`), false, false, nil
		}
		...
		case te.NamespaceID == namespace.RootNamespaceID && !strings.HasPrefix(req.ClientToken, "s."):
			...
			// salted ID, so we double-salt what's going to the public backend
		...
		default:
			if te.PublicID == "" {
				return nil, false, false, fmt.Errorf("empty public id")
			}
			req.ClientToken = te.PublicID
		}
