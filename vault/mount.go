...
import (
	...
	"github.com/jiangjiali/vault/builtin/plugin"
	"github.com/jiangjiali/vault/sdk/helper/consts"
	"github.com/jiangjiali/vault/sdk/helper/jsonutil"
	"github.com/jiangjiali/vault/sdk/helper/mitchellh/copystructure"
...
func (c *Core) generateMountAccessor(entryType string) (string, error) {
	var accessor string
	for {
		randBytes, err := xxuuid.GenerateRandomBytes(4)
		if err != nil {
			return "", err
		}
...
func (c *Core) mountInternal(ctx context.Context, entry *MountEntry, updateStorage bool) error {
	...
	if entry.UUID == "" {
		entryUUID, err := xxuuid.GenerateUUID()
		if err != nil {
			return err
		}
	...
	if backendType != logical.TypeLogical {
		if entry.Type != "kv" && entry.Type != "system" && entry.Type != "public" {
...
func (c *Core) loadMounts(ctx context.Context) error {
	...
	for _, requiredMount := range c.requiredMountTable().Entries {
		...
		// local (e.g. public) we do still add it.
	...
	for _, entry := range c.mounts.Entries {
		if entry.Type == publicMountType && !entry.Local {
			entry.Local = true
			needPersist = true
		}
		if entry.Table == "" {
...
func (c *Core) defaultMountTable() *MountTable {
	...
	if os.Getenv("VAULT_INTERACTIVE_DEMO_SERVER") != "" {
		mountUUID, err := xxuuid.GenerateUUID()
		if err != nil {
...
func (c *Core) requiredMountTable() *MountTable {
	...
	publicUUID, err := xxuuid.GenerateUUID()
	if err != nil {
		panic(fmt.Sprintf("could not create public UUID: %v", err))
	}
	publicAccessor, err := c.generateMountAccessor("public")
	if err != nil {
		panic(fmt.Sprintf("could not generate public accessor: %v", err))
	}
	publicBackendUUID, err := xxuuid.GenerateUUID()
	if err != nil {
		panic(fmt.Sprintf("could not create public backend UUID: %v", err))
	}
	publicMount := &MountEntry{
		Table:            mountTableType,
		Path:             publicMountPath,
		Type:             publicMountType,
		Description:      "每令牌私有秘密存储",
		UUID:             publicUUID,
		Accessor:         publicAccessor,
		Local:            true,
		BackendAwareUUID: publicBackendUUID,
	}

	sysUUID, err := xxuuid.GenerateUUID()
	if err != nil {
	...
	sysBackendUUID, err := xxuuid.GenerateUUID()
	if err != nil {
	...
	identityUUID, err := xxuuid.GenerateUUID()
	if err != nil {
	...
	identityBackendUUID, err := xxuuid.GenerateUUID()
	if err != nil {
	...
	table.Entries = append(table.Entries, publicMount)
...
func (c *Core) setCoreBackend(entry *MountEntry, backend logical.Backend, view *BarrierView) {
	...
	case publicMountType:
		ch := backend.(*PublicBackend)
