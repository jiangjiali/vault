...
import (
	...
	"github.com/jiangjiali/vault/sdk/helper/locksutil"
	"github.com/jiangjiali/vault/sdk/helper/mitchellh/mapstructure"
	...
	"github.com/jiangjiali/vault/sdk/helper/strutil"
	"github.com/jiangjiali/vault/sdk/logical"
	"github.com/jiangjiali/vault/sdk/plugin/pb"
...
var (
	...
	destroyPublic = func(ctx context.Context, ts *TokenStore, te *logical.TokenEntry) error {
		if ts.publicBackend == nil {
		...
		case te.NamespaceID == namespace.RootNamespaceID && !strings.HasPrefix(te.ID, "s."):
			...
			return ts.publicBackend.revoke(ctx, salt.SaltID(ts.publicBackend.saltUUID, saltedID, salt.SHA1Hash))

		default:
			if te.PublicID == "" {
				return fmt.Errorf("missing public ID while destroying")
			}
			return ts.publicBackend.revoke(ctx, te.PublicID)
		}
	}
)
...
type TokenStore struct {
	*framework.Backend
	...
	publicDestroyer func(context.Context, *TokenStore, *logical.TokenEntry) error
...
func (ts *TokenStore) create(ctx context.Context, entry *logical.TokenEntry) error {
	...
	case logical.TokenTypeDefault, logical.TokenTypeService:
		...
		if tokenNS.ID != namespace.RootNamespaceID || strings.HasPrefix(entry.ID, "s.") {
			if entry.PublicID == "" {
				publicID, err := base62.Random(TokenLength)
				if err != nil {
					return err
				}
...
func (ts *TokenStore) revokeInternal(ctx context.Context, saltedID string, skipOrphan bool) (ret error) {
	...
	err = ts.publicDestroyer(ctx, ts, entry)
	if err != nil {
		return err
	}
...
func (ts *TokenStore) handleTidy(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	...
	go func() {
		...
		doTidy := func() error {
			...
			// List all the public storage keys
			publicKeys, err := ts.publicBackend.storageView.List(quitCtx, "")
			if err != nil {
				return errwrap.Wrapf("failed to fetch public storage keys: {{err}}", err)
			}
			...
			deletedCountInvalidPublicKey int64

			validPublicKeys := make(map[string]bool)
			...
			for _, saltedAccessor := range saltedAccessorList {
				...
				case te == nil:
					...
					deletedCountAccessorInvalidToken++
				default:
					// Cache the public storage key when the token is valid
					switch {
					case te.NamespaceID == namespace.RootNamespaceID && !strings.HasPrefix(te.ID, "s."):
						...
						validPublicKeys[salt.SaltID(ts.publicBackend.saltUUID, saltedID, salt.SHA1Hash)] = true
					default:
						if te.PublicID == "" {
							tidyErrors = multierror.Append(tidyErrors, fmt.Errorf("missing public ID for a valid token"))
							continue
						}
						validPublicKeys[te.PublicID] = true
					}
				}
			}

			// Revoke invalid public storage keys
			for _, key := range publicKeys {
				countPublicKeys++
				if countPublicKeys%500 == 0 {
					ts.logger.Info("checking if there are invalid publics", "progress", countPublicKeys)
				}

				key := strings.TrimSuffix(key, "/")
				if !validPublicKeys[key] {
					ts.logger.Info("deleting invalid public", "key", key)
					err = ts.publicBackend.revoke(quitCtx, key)
					if err != nil {
						tidyErrors = multierror.Append(tidyErrors, errwrap.Wrapf(fmt.Sprintf("failed to revoke public key %q: {{err}}", key), err))
					}
					deletedCountInvalidPublicKey++
				}
			}
			...
			ts.logger.Info("number of deleted public keys that were invalid", "count", deletedCountInvalidPublicKey)
