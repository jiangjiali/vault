...
import (
	...
	"github.com/jiangjiali/vault/sdk/helper/identity/mfa"
	"github.com/jiangjiali/vault/sdk/helper/memdb"
	"github.com/jiangjiali/vault/sdk/helper/namespace"
	"github.com/jiangjiali/vault/sdk/helper/storagepacker"
...
func (i *IdentityStore) sanitizeAlias(ctx context.Context, alias *identity.Alias) error {
	...
	if alias.ID == "" {
		alias.ID, err = xxuuid.GenerateUUID()
		if err != nil {
...
func (i *IdentityStore) sanitizeEntity(ctx context.Context, entity *identity.Entity) error {
	...
	// Create an ID if there isn't one already
	if entity.ID == "" {
		entity.ID, err = xxuuid.GenerateUUID()
		if err != nil {
...
func (i *IdentityStore) sanitizeAndUpsertGroup(ctx context.Context, group *identity.Group, previousGroup *identity.Group, memberGroupIDs []string) error {
	...
	// Create an ID if there isn't one already
	if group.ID == "" {
		group.ID, err = xxuuid.GenerateUUID()
		if err != nil {
