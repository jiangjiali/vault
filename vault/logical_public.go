...
func PublicBackendFactory(ctx context.Context, conf *logical.BackendConfig) (logical.Backend, error) {
	...
	b.Backend = &framework.Backend{
		Help: strings.TrimSpace(publicHelp),
	}
...
// PublicBackend is used for storing secrets directly into the physical
...
func (b *PublicBackend) paths() []*framework.Path {
	return []*framework.Path{
		{
			...
			HelpSynopsis:    strings.TrimSpace(publicHelpSynopsis),
			HelpDescription: strings.TrimSpace(publicHelpDescription),
		},
	}
}

func (b *PublicBackend) revoke(ctx context.Context, saltedToken string) error {
	if saltedToken == "" {
...
func (b *PublicBackend) handleExistenceCheck(ctx context.Context, req *logical.Request, data *framework.FieldData) (bool, error) {
...
func (b *PublicBackend) handleRead(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	if req.ClientToken == "" {
...
func (b *PublicBackend) handleWrite(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	if req.ClientToken == "" {
		return nil, fmt.Errorf("client token empty")
	}
...
func (b *PublicBackend) handleDelete(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	if req.ClientToken == "" {
		return nil, fmt.Errorf("client token empty")
	}
...
func (b *PublicBackend) handleList(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	if req.ClientToken == "" {
		return nil, fmt.Errorf("client token empty")
	}
...
const publicHelp = `
The public backend reads and writes arbitrary secrets to the backend.
...
sharing possible (per-token publics). This can be useful for implementing
...
const publicHelpSynopsis = `
Pass-through secret storage to a token-specific public in the storage
backend, allowing you to read/write arbitrary data into secret storage.
`

const publicHelpDescription = `
The public backend reads and writes arbitrary data into secret storage,
encrypting it along the way.

The view into the public storage space is different for each token; it is
a per-token public. When the token is revoked all values are removed.
`
