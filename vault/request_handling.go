...
import (
	...
	"github.com/jiangjiali/vault/sdk/helper/errutil"
...
func (c *Core) handleCancelableRequest(ctx context.Context, ns *namespace.Namespace, req *logical.Request) (resp *logical.Response, err error) {
	...
	// public) -- did they want a key named foo/ or did they want to write
	...
	if wrapping {
		cubbyResp, cubbyErr := c.wrapInPublic(ctx, req, resp, auth)
		// If not successful, returns either an error response from the
		// public backend or an error; if either is set, set resp and err to
...
func (c *Core) handleRequest(ctx context.Context, req *logical.Request) (retResp *logical.Response, retAuth *logical.Auth, retErr error) {
	...
	if resp != nil &&
		req.Path == "public/response" &&
		len(te.Policies) == 1 &&
		te.Policies[0] == responseWrappingPolicyName {
		resp.AddWarning("Reading from 'public/response' is deprecated. Please use sys/wrapping/unwrap to unwrap responses, as it provides additional security checks and other benefits.")
	}
