...
import (
	...
	"github.com/jiangjiali/vault/sdk/helper/jose"
	squarejwt "github.com/jiangjiali/vault/sdk/helper/jose/jwt"
...
func (c *Core) wrapInPublic(ctx context.Context, req *logical.Request, resp *logical.Response, auth *logical.Auth) (*logical.Response, error) {
	if c.perfStandby {
...
DONELISTHANDLING:
	...
	if req.Path == "sys/wrapping/rewrap" {
		...
		httpResponse := logical.LResponseToHTTPResponse(resp)
	...
	if err := c.expiration.RegisterAuth(ctx, &te, wAuth); err != nil {
		// Revoke since it's not yet being tracked for expiration
		c.tokenStore.revokeOrphan(ctx, te.ID)
		c.logger.Error("failed to register public wrapping token lease", "request_path", req.Path, "error", err)
		return nil, ErrInternalError
	}
