...
import (
	...
	bplugin "github.com/jiangjiali/vault/sdk/plugin"
)
...
func (b *PBackend) reloadBackend(ctx context.Context) error {
...
func (b *PBackend) startBackend(ctx context.Context) error {
...
func (b *PBackend) HandleRequest(ctx context.Context, req *logical.Request) (*logical.Response, error) {
	b.RLock()
	canary := b.canary
	...
	if err != nil &&
		...
		if b.canary == canary {
			...
			b.canary, err = xxuuid.GenerateUUID()
			if err != nil {
				b.Unlock()
				return nil, err
			}
...
func (b *PBackend) HandleExistenceCheck(ctx context.Context, req *logical.Request) (bool, bool, error) {
	b.RLock()
	canary := b.canary
