...
import (
	...
	"github.com/jiangjiali/vault/sdk/helper/keysutil"
...
func (b *backend) pathSignWrite(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	...
	for i, item := range batchInputItems {
		...
		var xxcontext []byte
		if len(contextRaw) != 0 {
			xxcontext, err = base64.StdEncoding.DecodeString(contextRaw)
			if err != nil {
		...
		sig, err := p.Sign(ver, xxcontext, input, hashAlgorithm, sigAlgorithm, marshaling)
		if err != nil {
			if batchInputRaw != nil {
...
func (b *backend) pathVerifyWrite(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	...
	for i, item := range batchInputItems {
		...
		valid, err := p.VerifySignature(xxcontext, input, hashAlgorithm, sigAlgorithm, marshaling, sig)
		if err != nil {
			switch err.(type) {
